...
class StonescriptParser {
  ...
  // Rule 6: activate — validates against valid_ability_ids from JSON (case-insensitive)
  _checkActivate(arg) {
    if (!arg.trim()) return [];
    const val = arg.trim();
    // Multi-word is always invalid
    if (/\s/.test(val))
      return [this._warn(W.BAD_ACTIVATE,
        `'${val}' no es un ability ID valido (no puede contener espacios). ` +
        `Valores validos: ${[...this.validAbilityIds].join(', ')}.`)];
    // Must be a valid identifier (letters, digits, underscore only — no accents)
    if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(val))
      return [this._warn(W.BAD_ACTIVATE,
        `'${val}' contiene caracteres no validos. ` +
        `Un ability ID solo puede contener letras a-z A-Z, digitos y _.`)];
    // Check against known list (case-insensitive)
    const lc = val.toLowerCase();
    if (!this.validAbilityIds.has(lc))
      return [this._warn(W.BAD_ACTIVATE,
